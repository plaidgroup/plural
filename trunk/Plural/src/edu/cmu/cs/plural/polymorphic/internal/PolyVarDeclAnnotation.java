...
/**
 * An {@link ICrystalAnnotation} for the polymorphic variable
 * declaration annotations. There are three, and this one handles
 * all of them.
 * 
 ...
 *
 */
public class PolyVarDeclAnnotation implements ICrystalAnnotation {
	...
	/** What is the name of the polymorphic variable that is being declared? */
	public String getVariableName() {
		return (String)crystalAnnotation.getObject("value");
	}
	
	/** What kind of polymorphic variable is being declared? */
	public PolyVarKind getKind() {
		final String qual_name = this.getName();
		if( qual_name.equals(Exact.class.getName()) ) {
			return PolyVarKind.EXACT;
		}
		else if( qual_name.equals(Similar.class.getName()) ) {
			return PolyVarKind.SIMILAR;
		}
		else if( qual_name.equals(Symmetric.class.getName()) ) {
			return PolyVarKind.SYMMETRIC;
		}
		else {
			throw new RuntimeException("This should never happen. Only three kinds.");
		}
	}
}