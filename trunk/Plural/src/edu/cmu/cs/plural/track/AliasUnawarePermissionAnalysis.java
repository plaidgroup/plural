 ...
 *
 */
public class AliasUnawarePermissionAnalysis extends AbstractCrystalMethodAnalysis {
	 ...
	 * 
	 */
	public AliasUnawarePermissionAnalysis() {
		super();
	}
	...
	private class PermTransferFunction extends AbstractTACBranchSensitiveTransferFunction<TupleLatticeElement<Variable, Permissions>> {
		...
		@Override
		public IResult<TupleLatticeElement<Variable, Permissions>> transfer(ArrayInitInstruction instr, List<ILabel> labels, TupleLatticeElement<Variable, Permissions> value) {
			put(instr.getTarget(), value, Permission.createUnique("alive"));
			return LabeledSingleResult.createResult(value, labels);
		}

		private IResult<TupleLatticeElement<Variable, Permissions>> addImmutableTarget(AssignmentInstruction instr, List<ILabel> labels, TupleLatticeElement<Variable, Permissions> value) {
		...
		@Override
		public IResult<TupleLatticeElement<Variable, Permissions>> transfer(CastInstruction instr, List<ILabel> labels, TupleLatticeElement<Variable, Permissions> value) {
		...
		@Override
		public IResult<TupleLatticeElement<Variable, Permissions>> transfer(DotClassInstruction instr, List<ILabel> labels, TupleLatticeElement<Variable, Permissions> value) {
			// immutable permission to Class object
			return addImmutableTarget(instr, labels, value);
		}

		@Override
		public IResult<TupleLatticeElement<Variable, Permissions>> transfer(ConstructorCallInstruction instr, List<ILabel> labels, TupleLatticeElement<Variable, Permissions> value) {
			// TODO Auto-generated method stub
			return super.transfer(instr, labels, value);
		}

		@Override
		public IResult<TupleLatticeElement<Variable, Permissions>> transfer(CopyInstruction instr, List<ILabel> labels, TupleLatticeElement<Variable, Permissions> value) {
		...
		@Override
		public IResult<TupleLatticeElement<Variable, Permissions>> transfer(InstanceofInstruction instr, List<ILabel> labels, TupleLatticeElement<Variable, Permissions> value) {
			// immutable permission to boolean result
			return addImmutableTarget(instr, labels, value);
		}

		@Override
		public IResult<TupleLatticeElement<Variable, Permissions>> transfer(LoadLiteralInstruction instr, List<ILabel> labels, TupleLatticeElement<Variable, Permissions> value) {
			if(instr.isNull())
				// no permission to null value
				return super.transfer(instr, labels, value);
			// immutable permission for literal
			return addImmutableTarget(instr, labels, value);
		}

		@Override
		public IResult<TupleLatticeElement<Variable, Permissions>> transfer(LoadArrayInstruction instr, List<ILabel> labels, TupleLatticeElement<Variable, Permissions> value) {
			// TODO Auto-generated method stub
		...
		@Override
		public IResult<TupleLatticeElement<Variable, Permissions>> transfer(LoadFieldInstruction instr, List<ILabel> labels, TupleLatticeElement<Variable, Permissions> value) {
...
//					origin instanceof TypeVariable) {
				...
				if(fieldanno != null) {
					...
					if(originPerm == null)
						...
						put(target, value, fieldanno.getRequires().copyNewKind(PermissionKind.PURE));
					}
					else if(Permissions.isReadOnlyAccess(originPerm))
						// share becomes pure
						put(target, value, fieldanno.getRequires().purify());
					else
						// use share, imm, pure as is for modifying this-permission
						put(target, value, fieldanno.getRequires());
				}
...
//				problems.put(instr, "No support for access to foreign object: " + origin.getSourceString());
		...
		private SimplePermissionAnnotation fieldAnnotation(IVariableBinding field) {
			...
			for(ICrystalAnnotation a : 
				emptyForNull(analysisInput.getAnnoDB().getAnnosForField(field))) {
		...
		@Override
		public IResult<TupleLatticeElement<Variable, Permissions>> transfer(MethodCallInstruction instr, List<ILabel> labels, TupleLatticeElement<Variable, Permissions> value) {
			...
			for(ILabel label : labels) {
				if(label instanceof BooleanLabel) {
					...
					if(indicator != null) {
						TupleLatticeElement<Variable, Permissions> branchResult = ops.copy(value);
						...
						//if(Permissions.isConsistentState(indicatedPerms, newState))
							put(instr.getReceiverOperand(), branchResult, 
		...
		private SimplePermissionAnnotation resultPermission(IMethodBinding binding) {
			if(analysisInput.getAnnoDB().getSummaryForMethod(binding) == null)
				return null;
			for(ICrystalAnnotation a : 
				emptyForNull(analysisInput.getAnnoDB().getSummaryForMethod(binding).getReturn())) {
		...
		private List<SimplePermissionAnnotation> argumentPermissions(IMethodBinding binding, int argumentCount) {
			...
			for(int i = 0; i < argumentCount; i++) {
				String paramName = analysisInput.getAnnoDB().getSummaryForMethod(binding).getParameterName(i);
				SimplePermissionAnnotation anno = null;
				for(ICrystalAnnotation a : 
					emptyForNull(analysisInput.getAnnoDB().getSummaryForMethod(binding).getParameter(i))) {
		...
		@Override
		public IResult<TupleLatticeElement<Variable, Permissions>> transfer(NewArrayInstruction instr, List<ILabel> labels, TupleLatticeElement<Variable, Permissions> value) {
			put(instr.getTarget(), value, Permission.createUnique("alive"));
			return LabeledSingleResult.createResult(value, labels);
		}

		@Override
		public IResult<TupleLatticeElement<Variable, Permissions>> transfer(NewObjectInstruction instr, List<ILabel> labels, TupleLatticeElement<Variable, Permissions> value) {
			List<SimplePermissionAnnotation> argannos = argumentPermissions(instr.resolveBinding(), instr.getArgOperands().size());
			List<Variable> args = instr.getArgOperands();
			for(int i = 0; i < argannos.size(); i++) {
		...
		@Override
		public IResult<TupleLatticeElement<Variable, Permissions>> transfer(StoreArrayInstruction instr, List<ILabel> labels, TupleLatticeElement<Variable, Permissions> value) {
		...
		@Override
		public IResult<TupleLatticeElement<Variable, Permissions>> transfer(StoreFieldInstruction instr, List<ILabel> labels, TupleLatticeElement<Variable, Permissions> value) {
...
//					dest instanceof TypeVariable) {
				...
				if(fieldanno != null) {
					...
					put(src, value, Permissions.subtractPermissions(get(src, value), fieldanno));
				}
				// TODO what if field is not annotated?
//			}
//			else
