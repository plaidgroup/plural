 ...
 * when pure permissions are constructed by subtraction.
 */
@FailingTest(1)
@UseAnalyses("NIMBYChecker")
public class PureSubtractNoAtomic {
	...
	static void foo(@Unique(returned = false) PureSubtractNoAtomic psna) {
		psna.takeMyFull();
		
		if( psna.isOpen() )
			psna.mustBeOpen(); // Error
	}
}
