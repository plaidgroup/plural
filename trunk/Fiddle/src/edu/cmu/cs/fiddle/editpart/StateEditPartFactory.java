...
/**
 * Creates edit parts for their corresponding model elements.
 * 
...
public class StateEditPartFactory implements EditPartFactory {
	...
	public EditPart createEditPart(EditPart context, Object model) {
		...
		if( model instanceof StateMachine ) {
			editPart = new TopLevelEditPart();
		} else if( model instanceof Connection ) {
			editPart = new ConnectionEditPart();
		} else if( model instanceof InitialState ) {
		...
		} else if( model instanceof Dimension ) {
			editPart = new DimensionEditPart();
		}
